<?php

if (!defined('ABSPATH')) {
    exit;
}

class EventsDisplayWidget extends \Elementor\Widget_Base {
    
    public function get_name() {
        return 'events_display';
    }
    
    public function get_title() {
        return 'Events Display';
    }
    
    public function get_icon() {
        return 'eicon-calendar';
    }
    
    public function get_categories() {
        return ['general'];
    }
    
    protected function register_controls() {
        
        $this->start_controls_section(
            'content_section',
            [
                'label' => 'Content Settings',
                'tab' => \Elementor\Controls_Manager::TAB_CONTENT,
            ]
        );
        
        $this->add_control(
            'title',
            [
                'label' => 'Section Title',
                'type' => \Elementor\Controls_Manager::TEXT,
                'default' => 'Upcoming Events',
            ]
        );
        
        $this->add_control(
            'event_count',
            [
                'label' => 'Number of Events',
                'type' => \Elementor\Controls_Manager::NUMBER,
                'default' => 6,
                'min' => 1,
                'max' => 20,
            ]
        );
        
        $this->add_control(
            'show_upcoming_only',
            [
                'label' => 'Show Upcoming Events Only',
                'type' => \Elementor\Controls_Manager::SWITCHER,
                'default' => 'yes',
            ]
        );
        
        $this->add_control(
            'category_filter',
            [
                'label' => 'Category Filter',
                'type' => \Elementor\Controls_Manager::TEXT,
                'placeholder' => 'Leave empty for all categories',
            ]
        );
        
        $this->add_control(
            'layout_style',
            [
                'label' => 'Layout Style',
                'type' => \Elementor\Controls_Manager::SELECT,
                'default' => 'grid',
                'options' => [
                    'grid' => 'Grid',
                    'list' => 'List',
                    'timeline' => 'Timeline',
                    'calendar' => 'Calendar View',
                ],
            ]
        );
        
        $this->end_controls_section();
        
        $this->start_controls_section(
            'style_section',
            [
                'label' => 'Style',
                'tab' => \Elementor\Controls_Manager::TAB_STYLE,
            ]
        );
        
        $this->add_control(
            'card_background',
            [
                'label' => 'Card Background',
                'type' => \Elementor\Controls_Manager::COLOR,
                'default' => '#ffffff',
            ]
        );
        
        $this->add_control(
            'accent_color',
            [
                'label' => 'Accent Color',
                'type' => \Elementor\Controls_Manager::COLOR,
                'default' => '#007cba',
            ]
        );
        
        $this->add_control(
            'text_color',
            [
                'label' => 'Text Color',
                'type' => \Elementor\Controls_Manager::COLOR,
                'default' => '#333333',
            ]
        );
        
        $this->add_control(
            'border_radius',
            [
                'label' => 'Border Radius',
                'type' => \Elementor\Controls_Manager::SLIDER,
                'range' => [
                    'px' => [
                        'min' => 0,
                        'max' => 50,
                    ],
                ],
                'default' => [
                    'size' => 10,
                ],
            ]
        );
        
        $this->end_controls_section();
    }
    
    protected function render() {
        $settings = $this->get_settings_for_display();
        
        $widget_id = 'events-display-' . $this->get_id();
        ?>
        
        <div class="events-display-widget" id="<?php echo $widget_id; ?>">
            <?php if ($settings['title']): ?>
                <h2 class="events-section-title"><?php echo esc_html($settings['title']); ?></h2>
            <?php endif; ?>
            
            <div class="events-container <?php echo esc_attr($settings['layout_style']); ?>">
                <div class="events-loading">Loading events...</div>
                <div class="events-items"></div>
            </div>
        </div>
        
        <style>
            #<?php echo $widget_id; ?> .event-item {
                background: <?php echo esc_attr($settings['card_background']); ?>;
                border-radius: <?php echo esc_attr($settings['border_radius']['size']); ?>px;
                color: <?php echo esc_attr($settings['text_color']); ?>;
            }
            #<?php echo $widget_id; ?> .event-date {
                background: <?php echo esc_attr($settings['accent_color']); ?>;
                color: white;
            }
            #<?php echo $widget_id; ?> .event-category {
                color: <?php echo esc_attr($settings['accent_color']); ?>;
            }
        </style>
        
        <script>
        jQuery(document).ready(function($) {
            loadEventItems('<?php echo $widget_id; ?>', {
                limit: <?php echo intval($settings['event_count']); ?>,
                category: '<?php echo esc_js($settings['category_filter']); ?>',
                upcoming: <?php echo $settings['show_upcoming_only'] === 'yes' ? 'true' : 'false'; ?>,
                layout: '<?php echo esc_js($settings['layout_style']); ?>'
            });
        });
        </script>
        
        <?php
    }
}
